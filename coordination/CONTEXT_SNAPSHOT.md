...
Last updated after: TASK-080 acceptance and TASK-081 dispatch
...
`TASK-081` is dispatched as the next Phase 2.5 handoff.
...
- `TASK-081`: Hong Kong financial statements/indicators batch/report-period hardening with gated live smoke evidence.
...
For active TASK-081 specifically, execution may edit only the files listed in its handoff. It must not edit FeatureHub, Scanner, StrategyLab, BacktestEngine, portfolio, signal, risk, notification, AI, UI, or automated-trading modules; use credentials; or add hidden live network calls. TASK-081 requires default offline-safe tests plus a gated live-enabled smoke attempt.
