...
Last updated after: TASK-108 Review rejection and date-window rework dispatch
...
`TASK-108` remains active after Review Agent rejection. Active handoff: `coordination/handoffs/TASK-108_DATAHUB_A_SHARE_COMPANY_ANNOUNCEMENTS_DATE_WINDOW_REWORK.md`. The focused scope is a minimal DataHub A-share `DatasetName.COMPANY_ANNOUNCEMENTS` date-window/fallback truth rework: add live date-window assertions and prevent fallback per-day upstream/source availability failures from silently satisfying incomplete requested windows. Default tests must remain offline-safe, live smoke gated, route-signature/schema/payload/normalization defects hard failures, capability truth conservative unless fully proven, and downstream modules inactive.
...
The owner reopened DataHub as Phase 2.5-P before FeatureHub resumes and then upgraded all phase gates to the Personal Trading Perfection Standard. TASK-093 replaced the previous FeatureHub technical-indicator handoff with `coordination/handoffs/TASK-093_DATAHUB_PERSONAL_TRADING_READINESS_GATE.md` and is now closed after its follow-up queue rework. TASK-094 through TASK-107 are closed. TASK-108 remains active after Review rejection and is dispatched to 5.3 Execution for focused company-announcement date-window/fallback truth rework. It must not change FeatureHub, Scanner, StrategyLab, BacktestEngine, portfolio, signal, risk, AI, notification, UI, automated trading, paid credentials, or hidden default live network behavior.
...
For active TASK-108 specifically, the next role is 5.3 Execution. Expected write path is `coordination/reports/TASK-108_REPORT.md`. Execution must follow `coordination/handoffs/TASK-108_DATAHUB_A_SHARE_COMPANY_ANNOUNCEMENTS_BREADTH_HISTORY_HARDENING.md`, modifying only the allowed DataHub A-share company-announcement files and report. It must harden public-source announcement breadth/history/source-truth where stable no-credential routes expose it, keep capability truth conservative unless fully proven, preserve default offline safety and gated live behavior, preserve HKEX announcement behavior unless shared contract changes require narrow regression updates, and avoid downstream modules, paid credentials, controller-owned state, or hidden default live network behavior.
...
For active TASK-108 specifically, the next role is 5.3 Execution rework. Expected write path remains `coordination/reports/TASK-108_REPORT.md`. Execution must follow `coordination/handoffs/TASK-108_DATAHUB_A_SHARE_COMPANY_ANNOUNCEMENTS_DATE_WINDOW_REWORK.md`, modifying only the allowed DataHub A-share company-announcement files and report. It must add live date-window assertions, prevent fallback per-day upstream/source availability failures from silently satisfying incomplete requested windows, keep capability truth conservative, preserve default offline safety and gated live behavior, and avoid downstream modules, paid credentials, controller-owned state, or hidden default live network behavior.
