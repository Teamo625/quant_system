...
Last updated after: TASK-107 closure and TASK-108 dispatch
...
`TASK-108` is active as the next executable TASK-093 follow-up queue item. Active handoff: `coordination/handoffs/TASK-108_DATAHUB_A_SHARE_COMPANY_ANNOUNCEMENTS_BREADTH_HISTORY_HARDENING.md`. The focused scope is DataHub A-share `DatasetName.COMPANY_ANNOUNCEMENTS` breadth/history/source-truth hardening where stable no-credential public routes expose it. Default tests must remain offline-safe, live smoke gated, route-signature/schema/payload/normalization defects hard failures, HKEX announcement behavior preserved unless narrowly regression-protected, capability truth conservative unless fully proven, and downstream modules inactive.
...
The owner reopened DataHub as Phase 2.5-P before FeatureHub resumes and then upgraded all phase gates to the Personal Trading Perfection Standard. TASK-093 replaced the previous FeatureHub technical-indicator handoff with `coordination/handoffs/TASK-093_DATAHUB_PERSONAL_TRADING_READINESS_GATE.md` and is now closed after its follow-up queue rework. TASK-094 through TASK-107 are closed. TASK-108 is active and dispatched to 5.3 Execution for A-share company announcements breadth/history/source-truth hardening. It must not change FeatureHub, Scanner, StrategyLab, BacktestEngine, portfolio, signal, risk, AI, notification, UI, automated trading, paid credentials, or hidden default live network behavior.
