# TASK-059 Token-Required Retry Rework: DataHub Tushare index weight credentialed live PASS
...
- `coordination/handoffs/TASK-059_DATAHUB_TUSHARE_INDEX_WEIGHT_CREDENTIALED_LIVE_RETRY_REWORK.md`
...
- the latest retry truthfully stopped at the `TUSHARE_TOKEN` precondition gate
...
- `index_weight_history` must remain `planned` until a credentialed live PASS validates at least one schema-valid record

This handoff is only to resolve that Review finding. Do not broaden the task.

## Precondition

Run this token-required retry only after the operator supplies a valid `TUSHARE_TOKEN` in the execution environment.
...
TASK-059 must not be closed by Controller until a fresh Review Agent review accepts this token-required retry rework. Do not enter Integration during execution.
