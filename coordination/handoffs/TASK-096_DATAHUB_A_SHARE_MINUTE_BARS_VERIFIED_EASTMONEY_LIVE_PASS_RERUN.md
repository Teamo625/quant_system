# TASK-096 DataHub A-share Minute Bars Verified Eastmoney Live PASS Rerun
...
The rerun again showed the mandatory Eastmoney live gate is not met from the prior local environment:
...
Produce fresh TASK-096 live-enabled evidence from a host with verified end-to-end Eastmoney minute-bars API reachability or a verified working proxy path.
...
Expected write path if Eastmoney is reachable and the live smoke passes without repository defects:
...
1. Verify the live path before claiming a PASS-ready environment.
