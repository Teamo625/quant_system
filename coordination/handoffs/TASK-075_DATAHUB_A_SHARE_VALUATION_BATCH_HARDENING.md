...
TASK-071 audited DataHub against the trading-usable completion standard and found that most real-source capabilities remain narrow slices. TASK-072 closed the highest-priority A-share daily-bars batch gap. TASK-073 and TASK-074 added contract and bounded adapter coverage for A-share listing/delisting/ST/status history.

Phase 2.5 remains open because A-share medium/long-term research inputs are still not trading-usable enough. `a_share_valuation_history` remains `partial`; the existing `AkshareAShareValuationSnapshotAdapter` has accepted one-symbol live evidence but still rejects multi-symbol requests and has limited standardized historical/window behavior.
...
Harden AKShare-backed A-share `VALUATION_SNAPSHOT` access from a one-symbol slice into deterministic, caller-provided multi-symbol access with bounded date-window filtering where the public routes provide dated valuation records.

The adapter should preserve existing one-symbol behavior, reject invalid symbols clearly, normalize records to `DatasetName.VALUATION_SNAPSHOT`, keep default tests offline-safe, and include a gated live smoke that truthfully reports PASS, SKIP, or FAIL with root-cause evidence.

This task does not require full-market collection or full-history backfill.
...
- Extend `AkshareAShareValuationSnapshotAdapter` for multiple requested A-share symbols through the existing `SourceRequest.symbols` path.
- Preserve support for exactly one symbol.
- Keep `symbols=None` or an empty symbol list as a clear error; do not silently fetch a full-market valuation table.
...
- Fetch each requested symbol through bounded no-credential public AKShare valuation routes and combine normalized records.
- Honor `start_date` and `end_date` for every symbol using normalized `trade_date`; if a source route only provides a latest snapshot, apply the bounds deterministically and document that limitation.
- Preserve source-truth optionality for fields not reliably exposed by public routes. Do not invent valuation metrics.
- Keep records compatible with `DatasetRegistry.validate_record(DatasetName.VALUATION_SNAPSHOT, ...)`.
...
- Do not change `DatasetName.VALUATION_SNAPSHOT` schema in this task.
- Update only `a_share_valuation_history` capability truth if implementation and live evidence justify it. Keep it `partial` unless the adapter demonstrably satisfies the full trading-usable breadth/history standard; otherwise refine the gap text conservatively.
...
Write `coordination/reports/TASK-075_REPORT.md` with:
...
- multi-symbol A-share valuation requests work through the adapter contract in offline tests
