...
- TASK-123 remains active after Review rejection of the initial source-breadth result. The active rework must fix the bounded-request blocker by preventing default unbounded Sina full-table snapshot fetches for bounded scale/share requests, remove unrelated NAV adapter helper duplication unless directly justified, and keep `fund_scale_and_share` conservative without Controller closure until fresh Review acceptance.
