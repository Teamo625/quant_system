...
- TASK-115 is dispatched as the next executable Phase 2.5-P DataHub hardening task for `hk_corporate_actions`: expand HK corporate-action taxonomy/history coverage where stable no-credential routes prove source truth, or truthfully constrain capability/source wording without promotion. The preceding optional `hk_minute_bars` queue item remains owner-waiver-required and is not implementation-dispatched without owner waiver or explicit feasibility scope.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; the remaining `warn` items, including `hk_corporate_actions`, require DataHub hardening or explicit owner-accepted disposition before FeatureHub can reopen.
