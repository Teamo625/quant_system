...
- TASK-140 is dispatched as the next ordinary Phase 3-P FeatureHub capability-cluster handoff using readiness batch `featurehub_valuation_flow_batch_01`, covering `FH-VAL-001` and `FH-FLOW-001`: valuation PE/PB/PS-style outputs plus percentile/relative valuation, and capital/fund-flow rolling change, intensity, and normalization variants.
- TASK-140 is closed after accepted Review Agent verification. It added PE/PB/PS-style valuation outputs, bounded valuation percentile and relative-history helpers, main/northbound/fund-flow rolling changes and intensity/normalization helpers, kept default tests offline-safe, required no live evidence, and updated readiness so the next recommended batch is `featurehub_relative_features_batch_01`.
...
- TASK-152 initial Review rejected Controller closure on a focused duplicate-update validation gap: `merge_watchlist_snapshot()` and `merge_holding_snapshot()` accept duplicate symbols inside caller-provided `updates` inputs with last-write-wins behavior. A focused rework handoff, `coordination/handoffs/TASK-152_PORTFOLIO_SIGNAL_DUPLICATE_UPDATE_REWORK.md`, is dispatched. Phase 6 remains open, TASK-152 is not Done, and the rework must not be merged with later composition/risk or regression-readiness batches.
- TASK-152 is closed after accepted Review Agent verification of the focused duplicate-update validation rework. The current Phase 6 readiness gate reports `phase_closure_ready=false`, status counts `pass=3`, `warn=3`, `blocked=0`, `fail=0`, four remaining follow-up queue items, and two coherent follow-up batches. Phase 6 remains open because structured upstream-context signal composition, risk rule evaluation, and offline regression coverage for conflicts/staleness/risk blocks/lifecycle transitions are not yet all implemented.
- TASK-153 is dispatched from readiness batch `portfolio_signal_risk__personal_trading_hardening__batch_02` to implement local/offline structured signal composition and deterministic risk-rule foundation over caller-provided Scanner, StrategyLab, BacktestEngine, and portfolio context.
- TASK-153 initial Review rejected Controller closure on a focused risk-rule semantics gap: exposure, concentration, and market-constraint checks could silently treat actionable `ENTER` / `INCREASE` signals without sizing guidance as zero-change risk. A focused rework handoff, `coordination/handoffs/TASK-153_SIGNAL_RISK_RULE_NO_SIZING_REWORK.md`, is dispatched. Phase 6 remains open, TASK-153 is not Done, and the rework must not be merged with the later conflict/staleness/risk-block/lifecycle regression batch.
