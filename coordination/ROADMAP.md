...
- TASK-124 is dispatched as the next executable Phase 2.5-P DataHub hardening task for `fund_flow` breadth and history. It must prove stronger stable no-credential public ETF/fund flow metrics/history where feasible, or truthfully constrain capability/source wording without promotion.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; remaining `warn` items, including ETF/fund flow/premium-discount, index, sector/concept, macro/policy, and quality-report gaps, require DataHub hardening or explicit owner-accepted disposition before FeatureHub can reopen.
