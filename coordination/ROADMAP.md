...
- TASK-086 completed contract-only ETF/fund premium-discount source-fact schema coverage; `fund_premium_discount` now maps to `DatasetName.FUND_PREMIUM_DISCOUNT` and remains `partial` because adapter-backed public source evidence is still pending.
- TASK-087 is dispatched to implement bounded public ETF/fund `FUND_PREMIUM_DISCOUNT` adapter/source-fact coverage with default offline-safe tests and gated live smoke evidence.
