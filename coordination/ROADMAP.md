...
- TASK-100 remains open after Review rejection. The Baidu/Eastmoney overlap-conflict rework is directionally correct and has adequate offline overlap/gap coverage, but Review independently reran the gated live smoke and found current live truth is `FAIL` on the Baidu valuation route due to upstream non-JSON content while the report still recorded `PASS`. TASK-100 is re-dispatched for focused Baidu live failure/classifier/report truthfulness rework.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; the remaining `warn` items, including `a_share_valuation_history`, require DataHub hardening or explicit owner-accepted disposition before FeatureHub can reopen.
