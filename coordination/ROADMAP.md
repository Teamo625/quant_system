...
- TASK-128 is dispatched as the next executable capability-cluster handoff using DataHub readiness batch `sector_concept__datahub_hardening__sector_concept__batch_01`. It covers `sector_membership`, `sector_historical_changes`, and `sector_daily_bars`; it must prove stronger stable no-credential sector/concept membership history, classification-version/change-event truth, and sector daily-bar breadth/continuity where public routes expose them, or truthfully constrain capability/source wording without promotion.
- TASK-128 is not closed. Review rejected the first execution result because the changed sector daily-bar live smoke still catches broad `ValueError` failures and can mask repository-side defects as environment/source `SKIP`. Controller dispatched `coordination/handoffs/TASK-128_DATAHUB_SECTOR_DAILY_BAR_LIVE_CLASSIFIER_REWORK.md` as a minimal rework; it must not be merged with later readiness batches or ordinary hardening items.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; remaining `warn` items, including sector/concept, macro/policy, and quality-report gaps, require DataHub hardening or explicit owner-accepted disposition before FeatureHub can reopen.
