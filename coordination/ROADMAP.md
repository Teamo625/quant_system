...
- TASK-117 is dispatched as the next executable Phase 2.5-P DataHub hardening task for `hk_financial_data`: harden HK financial statements/indicators market breadth, report-period history, source-route/statement/metric-family truth, and date-window behavior where stable no-credential routes prove source truth, or truthfully constrain capability/source wording without promotion.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; the remaining `warn` items, including `hk_financial_data`, require DataHub hardening or explicit owner-accepted disposition before FeatureHub can reopen.
