...
- TASK-097 is active as the next unclosed TASK-093 queue item. It targets `a_share_adjustment_factors`, whose current non-pass reason is that adjustment-factor semantics are merged into generic `DatasetName.CORPORATE_ACTIONS` instead of a first-class symbol x date adjustment-factor source fact.
