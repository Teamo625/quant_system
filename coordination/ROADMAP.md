...
- TASK-106 is dispatched as the next executable TASK-093 queue item: A-share financial-statement breadth and history hardening for `DatasetName.FINANCIAL_STATEMENTS` where stable no-credential public routes expose source truth.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; the remaining `warn` items, including `a_share_financial_statements`, require DataHub hardening or explicit owner-accepted disposition before FeatureHub can reopen.
