...
- TASK-103 remains active after Review required rework. Initial execution added explicit A-share turnover/liquidity source-fact semantics, but Review found the dedicated live classifier too broad because `stock_zh_a_hist` route-signature/call-compatibility defects could become environment `SKIP`. A focused rework is dispatched to narrow the classifier, add regression evidence, and update the report before Controller closure can be considered.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; the remaining `warn` items, including `a_share_turnover_liquidity`, require DataHub hardening or explicit owner-accepted disposition before FeatureHub can reopen.
