...
- TASK-098 remains active after Review rejection. The initial A-share corporate-actions taxonomy hardening produced gated A-share live PASS evidence, but Review found a blocking shared-contract regression because the updated global `CORPORATE_ACTIONS` required fields are not yet satisfied by existing HK corporate-actions records. The active rework is `coordination/handoffs/TASK-098_DATAHUB_CORPORATE_ACTIONS_SHARED_CONTRACT_REWORK.md`.
