...
- TASK-108 initial review rejected Controller closure. The initial execution added bounded multi-symbol A-share company-announcement access and `source_route` truth, but Review found that live smoke coverage did not assert returned announcement dates were inside the requested bounded window and that fallback date-route handling could silently return partial requested-window history when per-day upstream/source availability failures were skipped. `coordination/handoffs/TASK-108_DATAHUB_A_SHARE_COMPANY_ANNOUNCEMENTS_DATE_WINDOW_REWORK.md` is dispatched for focused rework; TASK-108 remains active and must not close until fresh Review Agent acceptance.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; the remaining `warn` items, including `a_share_company_announcements`, require DataHub hardening or explicit owner-accepted disposition before FeatureHub can reopen.
