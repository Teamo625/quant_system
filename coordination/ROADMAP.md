...
- TASK-131 is dispatched as the next executable A-share capability-cluster handoff using readiness batch `a_share__datahub_hardening__a_share__batch_01`. It covers `a_share_listing_delisting_st_status`, `a_share_suspension_resumption`, `a_share_minute_bars`, `a_share_adjustment_factors`, `a_share_corporate_actions`, and `a_share_valuation_history`; it must strengthen stable no-credential public-source proof where feasible or truthfully constrain capability/catalog wording without promotion.
