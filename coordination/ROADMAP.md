...
- TASK-132 is active after Review rejection. The initial A-share capability-cluster execution used DataHub readiness batch `a_share__datahub_hardening__a_share__batch_02`, but Review found `a_share_northbound_flow` source truth overstated `stock_hsgt_individual_detail_em` as established fallback coverage while live-enabled evidence recorded that fallback as upstream-broken and unproven. The active handoff is the focused northbound fallback truth rework, not a new readiness batch.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; unresolved A-share, Hong Kong, ETF/fund, index, sector/concept, macro/policy, and source-capability warning batches still require accepted hardening or owner-accepted disposition before FeatureHub can reopen.
