...
- TASK-129 is dispatched as the next executable capability-cluster handoff using DataHub readiness batch `macro_policy__datahub_hardening__macro_policy__batch_01`. It covers `macro_observations`, `macro_indicator_definitions`, `macro_release_metadata`, `policy_documents`, and `company_announcements_cross_market`; it must prove stronger stable no-credential macro/policy/announcement breadth, release/revision metadata, policy-document authority/history depth, and cross-market announcement parity where public routes expose them, or truthfully constrain capability/source wording without promotion.
