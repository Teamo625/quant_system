...
- TASK-109 is dispatched as the next executable TASK-093 queue item: A-share `DatasetName.MAJOR_ACTIVITY_EVENTS` breadth and history hardening beyond the current bounded block-trade detail route where stable no-credential public routes expose source truth.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; the remaining `warn` items, including `a_share_major_activity_events`, require DataHub hardening or explicit owner-accepted disposition before FeatureHub can reopen.
