...
- TASK-125 remains active after Review rejected the initial ETF/fund premium-discount breadth/history result. The active rework must fix the historical premium-discount live classifier so route/function-name tokens alone do not downgrade route-signature or call-compatibility defects into environment `SKIP`, add focused regression coverage, and receive fresh Review before Controller closure.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; remaining `warn` items, including ETF/fund premium-discount, index, sector/concept, macro/policy, and quality-report gaps, require DataHub hardening or explicit owner-accepted disposition before FeatureHub can reopen.
