...
- TASK-096 is active as the next executable TASK-093 queue item. It targets `a_share_minute_bars` history continuity and broader public-source breadth beyond the bounded date-window coverage proven by TASK-078. Initial Review required focused rework because the new `1`-minute retention guard used fixed calendar days instead of source-backed trading-day retention, and the live-enabled Eastmoney smoke still skipped on proxy/connectivity availability.
