...
- TASK-102 is dispatched as the next executable TASK-093 queue item: A-share northbound-flow contract profile hardening for a dedicated DataHub contract/profile where stable no-credential public routes expose source truth.
- TASK-102 initial Review requires focused rework before closure. The dedicated northbound live classifier can currently downgrade AKShare route-signature/call-compatibility defects on `stock_hsgt_individual_em` to environment `SKIP`, so the active handoff is now the live-classifier truthfulness rework.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; the remaining `warn` items, including `a_share_northbound_flow`, require DataHub hardening or explicit owner-accepted disposition before FeatureHub can reopen.
