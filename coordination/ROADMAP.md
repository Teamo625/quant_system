...
- TASK-130 is dispatched as the next executable DataHub quality-report handoff using readiness batch `quality_reports__datahub_hardening__source__batch_01`. It covers `source_coverage_metadata`; it must enhance local-only quality-report coverage KPI metadata so personal-readiness gaps are visible in deterministic `DATA_QUALITY_REPORT` records without introducing live network behavior or downstream module work.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; the quality-report coverage KPI gap requires accepted hardening or explicit owner-accepted disposition before FeatureHub can reopen.
