...
- TASK-104 is dispatched as the next executable TASK-093 queue item: A-share limit-up/down breadth and history hardening for `DatasetName.LIMIT_UP_DOWN_EVENTS` where stable no-credential public routes expose source truth. Its initial Review requires focused rework because new `gettopicpreviouspool` / `gettopiczbgcpool` route-name tokens can mask repository-side payload/schema/normalization defects as environment `SKIP`; `TASK-104_DATAHUB_A_SHARE_LIMIT_UP_DOWN_LIVE_CLASSIFIER_REWORK.md` is active and TASK-104 is not closed.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; the remaining `warn` items, including `a_share_limit_up_down`, require DataHub hardening or explicit owner-accepted disposition before FeatureHub can reopen.
