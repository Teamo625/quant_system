...
- TASK-096 is active as the next executable TASK-093 queue item. It targets `a_share_minute_bars` history continuity and broader public-source breadth beyond the bounded date-window coverage proven by TASK-078. Initial Review required focused rework because the new `1`-minute retention guard used fixed calendar days instead of source-backed trading-day retention, and the live-enabled Eastmoney smoke still skipped on proxy/connectivity availability. The retention rework is now accepted for scope, and the subsequent live-rerun reports are accepted as truthful, but Controller closure remains blocked because the live-enabled Eastmoney minute-bars API smoke still records `SKIP` from proxy/connectivity; `coordination/handoffs/TASK-096_DATAHUB_A_SHARE_MINUTE_BARS_CONFIRMED_EASTMONEY_LIVE_PASS_RERUN.md` is dispatched from a host with confirmed `push2his.eastmoney.com` minute-bars API reachability or a verified working proxy path.
