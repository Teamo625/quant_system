...
- TASK-112 is dispatched as the next Phase 2.5-P DataHub hardening task continuing the unresolved `hk_universe_reference` item: diagnose the skipped bounded HK current-list live path, prove a stable no-credential list route or fallback if feasible, or truthfully constrain capability/source wording without promotion.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; the remaining `warn` items, including `hk_universe_reference`, require DataHub hardening or explicit owner-accepted disposition before FeatureHub can reopen.
