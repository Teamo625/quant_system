...
- TASK-120 is dispatched as the next Phase 2.5-P DataHub hardening task for ETF/fund NAV breadth/history truth. It must strengthen `fund_nav` where stable no-credential public routes expose broader source-backed NAV history or truthfully constrain capability/source wording without promotion.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; the remaining `warn` items, including ETF/fund NAV/holdings/scale/flow/premium-discount, index, sector/concept, macro/policy, and quality-report gaps, require DataHub hardening or explicit owner-accepted disposition before FeatureHub can reopen.
