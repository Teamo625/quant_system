...
- TASK-089 is dispatched to harden public index `INDEX_CONSTITUENTS` access from a one-index slice to caller-provided multi-index bounded constituent access and improve rebalance/effective-date metadata where public source truth permits, with default offline-safe tests and gated live smoke evidence.
