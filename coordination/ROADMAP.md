...
- TASK-139 remains open after the second Review rejection. The only remaining blocker is missing direct invalid `long_window` regression coverage for `calculate_macd()` plus an overstated report item. The active rework handoff is `coordination/handoffs/TASK-139_FEATUREHUB_MACD_LONG_WINDOW_TEST_REWORK.md`; Phase 3-P must not advance to the next readiness batch until this focused rework receives fresh Review acceptance.
