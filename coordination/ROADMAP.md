...
- TASK-122 remains active after Review rejected the initial ETF/fund scale/share canonical schema result. The initial execution added a first-class `FUND_SCALE_SHARE_SNAPSHOT` contract and kept `fund_scale_and_share` conservative, but Review blocked closure because the contract rejected valid negative scale/share change facts. The active rework handoff must fix signed metric semantics and focused offline tests without expanding into adapter work or promotion.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` requires owner waiver or future feasibility scope; the active TASK-122 Review blocker plus remaining `warn` items, including ETF/fund scale/flow/premium-discount, index, sector/concept, macro/policy, and quality-report gaps, require DataHub hardening or explicit owner-accepted disposition before FeatureHub can reopen.
