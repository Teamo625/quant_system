...
- TASK-090 is dispatched to harden public sector `SECTOR_MEMBERSHIP` access from a one-sector slice to caller-provided multi-sector bounded membership access and preserve membership date/history metadata where public source truth permits, with default offline-safe tests and gated live smoke evidence.
