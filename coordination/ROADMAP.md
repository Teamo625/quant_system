...
- TASK-135 is dispatched as the next adjacent Phase 2.5-P blocker-disposition handoff using DataHub readiness batch `hong_kong__owner_waiver_required__hong_kong_hong_kong_capability_readiness_hk_minute_bars__batch_01`. This is a single-item owner-waiver/blocker task for `hk_minute_bars`, not a normal hardening cluster, because the optional capability has no current dataset mapping and requires HK minute-bar source/contract feasibility evidence or explicit owner-waiver/blocker disposition before later ordinary hardening batches.
- Phase 2.5-P remains open. `index_weight_history` remains an owner credential blocker; optional `hk_minute_bars` is now the active owner-waiver/blocker disposition; unresolved ETF/fund, index, sector/concept, macro/policy, and source-capability warning batches still require accepted hardening or owner-accepted disposition before FeatureHub can reopen.
