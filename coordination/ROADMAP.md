...
- TASK-131 is active. Its original A-share capability-cluster execution was rejected by Review pending a focused source-catalog truth rework: the AKShare source-family notes must not claim BaoStock minute-bar history, because BaoStock coverage belongs to the separate `baostock_public_cn` source family. The active handoff is now `coordination/handoffs/TASK-131_DATAHUB_A_SHARE_SOURCE_CATALOG_TRUTH_REWORK.md`; TASK-131 must not close until fresh Review accepts the rework.
