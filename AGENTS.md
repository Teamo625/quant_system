...
The project is intentionally built in phases. The current allowed implementation scope is Phase 4-P Scanner Personal Trading Perfection Re-Review only. Downstream modules remain inactive until the controller explicitly reopens their phase after prerequisite hardening.
...
Current implementation phase: Phase 4-P Scanner Personal Trading Perfection Re-Review.
...
- `quant/datahub/` for implementation changes, unless the controller explicitly reopens a DataHub rework or blocker task
- `quant/features/` for implementation changes, unless the controller explicitly reopens a FeatureHub rework or blocker task
- `quant/strategies/`
- `quant/backtest/`
- `quant/portfolio/`
- `quant/notification/`
...
Do not implement trading strategies, backtest execution, portfolio/signal/risk logic, AI reports, push notifications, automated trading, or complex UI until the corresponding phase or sub-scope is reopened by the controller. In the current Phase 4-P scope, Scanner work must be limited to personal trading perfection re-review gates and explicitly dispatched hardening handoffs across existing Scanner domains. DataHub and FeatureHub are treated as accepted upstream public-source/no-paid scope baselines; do not modify DataHub or FeatureHub implementation files unless the controller explicitly reopens a focused upstream rework or blocker task.

All current and future phases, including historically completed foundation phases, must be re-reviewed against the Personal Trading Perfection Standard before downstream phases may rely on them as final. TASK-143 is the Scanner entry point for that re-review.
...
For real-source adapter or real data-fetching tasks, live smoke coverage is mandatory even though it must remain skipped by default. Phase 4-P tasks must audit or harden Scanner personal trading perfection without implementing StrategyLab, BacktestEngine, signal, risk, portfolio, AI, notification, UI, automated trading logic, FeatureHub calculations, or DataHub source adapters. Paid or private-credential requirements must be classified as blocked unless the owner explicitly provides credentials and opens that scope.
