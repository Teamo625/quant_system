...
class HkexHKTradingCalendarLiveClassifierTests(unittest.TestCase):
    def test_classifier_treats_network_errors_as_environment_issue(self) -> None:
        self.assertTrue(
            _is_live_environment_unavailable(
                OSError(111, "connection refused to www.hkex.com.hk")
            )
        )

    def test_classifier_keeps_contract_failures_as_non_environment_issue(self) -> None:
        self.assertFalse(_is_live_environment_unavailable(ValueError("Invalid trade date value")))


class HkexHKTradingCalendarLiveTests(unittest.TestCase):
    @unittest.skipUnless(
        LIVE_TESTS_ENABLED,
        "Live source tests are disabled. Set QUANT_SYSTEM_LIVE_TESTS=1 to enable.",
    )
    def test_live_hkex_hk_trading_calendar_smoke(self) -> None:
        adapter = HkexHKTradingCalendarAdapter()
        registry = DatasetRegistry()

        range_start = date.today().replace(month=1, day=1)
        ...
        try:
            ...
            if _is_live_environment_unavailable(exc):
                self.skipTest(
                    "live HKEX trading-calendar source unavailable in current environment: "
        ...
        if result.record_count < 1:
            self.skipTest(
                "live HKEX trading-calendar source returned no usable bounded sample records"
            )

        first_record = result.normalized_records[0]
        issues = registry.validate_record(DatasetName.TRADING_CALENDAR, first_record)
