...
class HkexCompanyAnnouncementsLiveClassifierTests(unittest.TestCase):
    def test_classifier_treats_network_errors_as_environment_issue(self) -> None:
        self.assertTrue(
            _is_live_environment_unavailable(
                OSError(111, "connection refused to www1.hkexnews.hk")
            )
        )

    def test_classifier_keeps_contract_failures_as_non_environment_issue(self) -> None:
        self.assertFalse(_is_live_environment_unavailable(ValueError("Invalid HK symbol value")))


class HkexCompanyAnnouncementsLiveTests(unittest.TestCase):
    @unittest.skipUnless(
        LIVE_TESTS_ENABLED,
        "Live source tests are disabled. Set QUANT_SYSTEM_LIVE_TESTS=1 to enable.",
    )
    def test_live_hkex_company_announcements_smoke(self) -> None:
        adapter = HkexCompanyAnnouncementsAdapter(max_records_without_symbols=20)
        ...
        try:
            ...
            if _is_live_environment_unavailable(exc):
                self.skipTest(
                    "live HKEX company-announcements source unavailable in current environment: "
                    f"{type(exc).__name__}: {exc}"
                )
            raise

        if result.record_count < 1:
            self.skipTest("live HKEX company-announcements source returned no usable records")
