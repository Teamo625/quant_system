...
class AkshareAShareSuspensionResumptionLiveClassifierTests(unittest.TestCase):
    def test_classifier_marks_network_related_errors_as_environment_unavailable(self) -> None:
        self.assertTrue(
            _is_live_environment_unavailable(
                RuntimeError(
                    "AKShare suspension/resumption route unavailable: ProxyError: proxy down"
                )
            )
        )
    ...
    def test_classifier_keeps_contract_failures_as_non_environment_issue(self) -> None:
        self.assertFalse(
            _is_live_environment_unavailable(ValueError("Invalid event_date value"))
        )
...
class AkshareAShareSuspensionResumptionLiveTests(unittest.TestCase):
    @unittest.skipUnless(
        LIVE_TESTS_ENABLED,
        "Live source tests are disabled. Set QUANT_SYSTEM_LIVE_TESTS=1 to enable.",
    )
    def test_live_akshare_a_share_suspension_resumption_smoke(self) -> None:
        try:
            import akshare as _ak  # noqa: F401
        except Exception as exc:
            self.skipTest(f"akshare is not available for live smoke test: {exc}")

        adapter = AkshareAShareSuspensionResumptionAdapter()
        ...
        for days_back in range(1, 31):
            ...
            try:
                ...
                if _is_live_environment_unavailable(exc) or adapter._is_suspension_resumption_route_unavailable(  # pylint: disable=protected-access
                    exc
                ):
                    self.skipTest(
                        "live AKShare A-share suspension/resumption source unavailable in "
