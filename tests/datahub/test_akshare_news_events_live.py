    ...
    def test_live_akshare_news_events_smoke(self) -> None:
        ...
        try:
            ...
            if _is_live_environment_unavailable(exc):
                self.skipTest(
                    "live AKShare news-events source unavailable in current environment: "
                    f"{type(exc).__name__}: {exc}"
                )
            raise

        if result.record_count < 1:
            self.skipTest("live AKShare news-events source returned no usable bounded sample records")

        first_record = result.normalized_records[0]
        issues = registry.validate_record(DatasetName.NEWS_EVENTS, first_record)
        self.assertEqual(issues, ())
        self.assertEqual(first_record["source"], AKSHARE_SOURCE_ID)
        self.assertIn("news_id", first_record)
        self.assertIn("title", first_record)
        self.assertIn("publish_time", first_record)
