...
class AkshareAShareTradingCalendarLiveTests(unittest.TestCase):
    @unittest.skipUnless(
        LIVE_TESTS_ENABLED,
        "Live source tests are disabled. Set QUANT_SYSTEM_LIVE_TESTS=1 to enable.",
    )
    def test_live_akshare_trading_calendar_smoke(self) -> None:
        try:
        ...
        try:
            ...
            if _is_live_environment_unavailable(exc):
                self.skipTest(
                    "live AKShare calendar source unavailable in current environment: "
        ...
        if result.record_count < 1:
            self.skipTest(
                "live AKShare calendar source returned no usable bounded sample records"
            )
