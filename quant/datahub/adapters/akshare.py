...
class AkshareAShareFinancialDataAdapter:
    ...
    _STATEMENT_FIELD_CANDIDATES: dict[str, dict[str, tuple[str, ...]]] = {
        "balance_sheet": {
            "total_assets": ("资产总计", "资产总额", "总资产", "资产合计"),
            "total_liabilities": ("负债合计", "负债总计", "负债总额", "总负债"),
        },
        "income_statement": {
            "revenue": ("营业收入", "营业总收入", "营业额", "总营业收入"),
            "net_profit": (
                "归属于母公司的净利润",
                "归属母公司所有者的净利润",
                "归属于母公司股东的净利润",
    ...
    _INDICATOR_META_FIELDS: frozenset[str] = frozenset(
        {
            "SECUCODE",
            "SECURITY_CODE",
            "SECURITY_NAME_ABBR",
    ...
    _INDICATOR_METRIC_NAME_MAP: dict[str, str] = {
        "EPSJB": "基本每股收益",
        "BPS": "每股净资产",
        "MGJYXJJE": "每股经营现金流",
    ...
    _CURRENCY_PER_SHARE_FIELDS: frozenset[str] = frozenset(
        {
            "EPSJB",
            "EPSKCJB",
            "EPSXS",
    ...
    def fetch(
        ...
        *,
        ...
        if dataset not in {
            ...
            raise ValueError(
                "Unsupported dataset for AkshareAShareFinancialDataAdapter: "
                f"{dataset.value}"
            )
    ...
    def _fetch_financial_statements(
        ...
        *,
        ...
        for statement_input, statement_type in self._STATEMENT_SPECS:
            try:
                payload = self._call_financial_report_route(
                    fetch_fn=fetch_fn,
                    stock_input=route_stock,
                ...
                if self._is_a_share_financial_route_unavailable(exc):
                    raise RuntimeError(
                        ...
                        f"{self._STATEMENT_ROUTE_NAME}(stock={route_stock}, "
                        f"symbol={statement_input}) -> {type(exc).__name__}: {exc}"
                    ) from exc
                raise

            rows = self._payload_to_rows(
                payload=payload,
                route_name=f"{self._STATEMENT_ROUTE_NAME}(symbol={statement_input})",
            )
            for row_idx, row in enumerate(rows):
                report_period_end = self._normalize_date(
                    self._pick(row, row_idx, "报告日", "REPORT_DATE", "报告期", "日期"),
                ...
                if source_ts is not None:
                    normalized_source_ts = self._normalize_datetime(source_ts, field_name="source_ts")
                    current_source_ts = existing.get("source_ts")
                    if current_source_ts is None or normalized_source_ts > str(current_source_ts):
                        existing["source_ts"] = normalized_source_ts

                currency = self._pick_optional(row, "CURRENCY", "币种")
                if currency is not None:
                    normalized_currency = self._normalize_currency(currency)
                    if "currency" in existing and existing["currency"] != normalized_currency:
                        raise ValueError(
                            "Conflicting duplicate A-share financial statement currency detected: "
                            f"symbol={symbol!r}, period={report_period_end!r}, "
                            f"statement_type={statement_type!r}, "
                            f"existing={existing['currency']!r}, "
                            f"candidate={normalized_currency!r}."
                        )
                    existing["currency"] = normalized_currency

                field_candidates = self._STATEMENT_FIELD_CANDIDATES.get(statement_type, {})
                for field_name, source_keys in field_candidates.items():
                    source_value = self._pick_optional(row, *source_keys)
                    if source_value is None:
                        continue
                    numeric_value = self._to_float(source_value, field_name=field_name)
    ...
    def _fetch_financial_indicators(
        ...
        *,
        ...
        fetch_fn = self._resolve_fetch_financial_indicator()
        try:
            ...
            if self._is_a_share_financial_route_unavailable(exc):
                raise RuntimeError(
                    "AKShare A-share financial indicators route unavailable: "
                    f"{self._INDICATOR_ROUTE_NAME}(indicator={self._INDICATOR_ROUTE_INPUT}) -> "
                    f"{type(exc).__name__}: {exc}"
                ) from exc
            raise
        ...
        rows = self._payload_to_rows(
            payload=payload,
            route_name=self._INDICATOR_ROUTE_NAME,
        )
        records_by_identity: dict[tuple[str, str, str], dict[str, Any]] = {}
        ingested_at = self._now_fn().isoformat()
        schema_version = self._registry.get(dataset).schema_version
        ...
        for row_idx, row in enumerate(rows):
            report_period_end = self._normalize_date(
                self._pick(row, row_idx, "REPORT_DATE", "报告期", "报告日", "日期"),
                field_name="report_period_end",
            )
            period_type = self._resolve_period_type(
                report_period_end=report_period_end,
                report_type=self._pick_optional(row, "REPORT_TYPE", "REPORT_DATE_NAME", "报告类型"),
            )
            ...
            for metric_key in sorted(row.keys()):
                metric_code = str(metric_key)
                if metric_code in self._INDICATOR_META_FIELDS:
                    continue
                ...
                metric_value = self._to_float(value, field_name=metric_code.lower())
                identity = (report_period_end, period_type, metric_code)
                existing = records_by_identity.get(identity)
                if existing is None:
                    record: dict[str, Any] = {
                        "symbol": symbol,
                    ...
                    if unit is not None:
                        record["unit"] = unit
                    if normalized_source_ts is not None:
                        record["source_ts"] = normalized_source_ts
                    records_by_identity[identity] = record
                    continue

                if float(existing["metric_value"]) != metric_value:
                    raise ValueError(
                        "Conflicting duplicate A-share financial indicator row detected: "
                        f"symbol={symbol!r}, period={report_period_end!r}, "
                        f"period_type={period_type!r}, metric_code={metric_code!r}, "
                        f"existing={existing['metric_value']!r}, candidate={metric_value!r}."
                    )
    ...
    def _resolve_fetch_financial_report(self) -> Callable[..., Any]:
        if self._fetch_financial_report is not None:
            return self._fetch_financial_report
        ...
        if hasattr(ak, self._STATEMENT_ROUTE_NAME):
            return getattr(ak, self._STATEMENT_ROUTE_NAME)
        raise RuntimeError(
    ...
    def _resolve_fetch_financial_indicator(self) -> Callable[..., Any]:
        if self._fetch_financial_indicator is not None:
            return self._fetch_financial_indicator
        ...
        if hasattr(ak, self._INDICATOR_ROUTE_NAME):
            return getattr(ak, self._INDICATOR_ROUTE_NAME)
        raise RuntimeError(
            "AKShare A-share financial indicators function is unavailable: "
            f"{self._INDICATOR_ROUTE_NAME}"
        )

    def _call_financial_report_route(
        self,
        *,
        fetch_fn: Callable[..., Any],
        stock_input: str,
        ...
        accepted_args, supports_var_kwargs = self._inspect_callable(fetch_fn)
        kwargs: dict[str, Any] = {}
        stock_arg = self._resolve_supported_arg(
            accepted_args=accepted_args,
            supports_var_kwargs=supports_var_kwargs,
            candidates=("stock", "code", "ts_code", "symbol"),
            route_name=self._STATEMENT_ROUTE_NAME,
            field_label="stock/code",
        )
        kwargs[stock_arg] = stock_input

        statement_candidates: tuple[str, ...] = ("symbol", "statement", "report_type", "table")
        if stock_arg == "symbol":
            statement_candidates = ("statement", "report_type", "table")
        statement_arg = self._resolve_supported_arg(
            accepted_args=accepted_args,
            supports_var_kwargs=supports_var_kwargs,
            candidates=statement_candidates,
            route_name=self._STATEMENT_ROUTE_NAME,
            field_label="statement symbol",
        )
        kwargs[statement_arg] = statement_input
        return fetch_fn(**kwargs)

    def _call_financial_indicator_route(
        self,
        *,
        fetch_fn: Callable[..., Any],
        symbol_input: str,
        ...
        symbol_arg = self._resolve_supported_arg(
            accepted_args=accepted_args,
            supports_var_kwargs=supports_var_kwargs,
            candidates=("symbol", "stock", "code", "ts_code"),
            route_name=self._INDICATOR_ROUTE_NAME,
        ...
        if self._supports_arg(
            ...
            kwargs["indicator"] = indicator_input
        elif self._supports_arg(
            "period",
            accepted_args=accepted_args,
            supports_var_kwargs=supports_var_kwargs,
        ):
            kwargs["period"] = indicator_input
        return fetch_fn(**kwargs)
        ...
        *,
        ...
        for arg_name in candidates:
            if self._supports_arg(
                ...
                return arg_name
        raise RuntimeError(
            "AKShare A-share financial-data route does not accept required argument: "
            f"route={route_name}, field={field_label}"
        )
    ...
    def _payload_to_rows(
        ...
        *,
        ...
        for idx, row in enumerate(candidate):
            if not isinstance(row, Mapping):
                raise ValueError(
                    "AKShare A-share financial-data payload row must be mapping. "
                    f"route={route_name}, idx={idx}, got={type(row).__name__}."
                )
            rows.append(row)
        return rows
    ...
    def _normalize_a_share_symbol(self, value: str) -> tuple[str, str, str]:
        ...
        if "." in value:
            ...
            if market not in {"SH", "SZ", "BJ"}:
                raise ValueError(
                    "Unsupported symbol market suffix for A-share financial-data adapter: "
                    f"{market!r}. Expected SH/SZ/BJ."
                )
            if not code.isdigit() or len(code) != 6:
                raise ValueError(
        ...
        raise ValueError(
            "Unsupported symbol format for A-share financial-data adapter: "
            f"{value!r}. Expected canonical like '600000.SH' or raw 6-digit stock code."
        )
    ...
    def _infer_market_from_code(self, code: str) -> str:
        ...
        if code.startswith(("0", "3")):
            if code.startswith("399"):
                raise ValueError(
                    "Index symbol is unsupported for A-share financial-data adapter: "
                    f"{code!r}."
                )
            return "SZ"
        if code.startswith(("4", "8", "9")):
        ...
        *keys: str,
        ...
        for key in keys:
            if key in row:
                return row[key]
        raise ValueError(
            "Missing required source field in A-share financial-data row "
            f"{row_idx}: one of {keys!r}."
        )
    ...
    def _resolve_period_type(
        ...
        *,
        ...
        if isinstance(report_type, str):
            ...
            if "年" in normalized:
                return "annual"
            if "半年" in normalized or "中报" in normalized:
                return "semiannual"
            if "季" in normalized:
                return "quarterly"
    ...
    def _resolve_indicator_unit(
        ...
        *,
        ...
        if metric_code in self._PERCENT_LIKE_FIELDS or metric_code.endswith("TZ"):
            return "percent"
        if metric_code in self._CURRENCY_PER_SHARE_FIELDS:
            if currency is None:
    ...
    def _normalize_currency(self, value: Any) -> str:
        if not isinstance(value, str):
            raise ValueError(
                "Invalid currency value type for A-share financial-data adapter: "
                f"{type(value).__name__}"
            )
        currency = value.strip().upper()
        if currency == "":
            raise ValueError("Invalid currency value: empty string")
        return currency
    ...
    def _filter_records_by_date(
        ...
        *,
        ...
        filtered: list[dict[str, Any]] = []
        for record in records:
            report_period_end = date.fromisoformat(str(record["report_period_end"]))
            if start_date is not None and report_period_end < start_date:
                continue
            if end_date is not None and report_period_end > end_date:
                continue
            filtered.append(dict(record))
        return filtered
    ...
    def _is_a_share_financial_route_unavailable(self, exc: BaseException) -> bool:
        network_exception_names = {
            "ProxyError",
            "ConnectionError",
            "ConnectTimeout",
...
class AkshareHKFinancialDataAdapter:
    ...
    _STATEMENT_ROUTE_NAME = "stock_financial_hk_report_em"
    _INDICATOR_ROUTE_NAME = "stock_financial_hk_analysis_indicator_em"
    ...
    _FINANCIAL_META_FIELDS: frozenset[str] = frozenset(
        {
            "SECUCODE",
            "SECURITY_CODE",
            "SECURITY_NAME_ABBR",
    ...
    def fetch(
        ...
        *,
        ...
        if dataset not in {
            ...
            raise ValueError(
                "Unsupported dataset for AkshareHKFinancialDataAdapter: "
                f"{dataset.value}"
            )
        ...
        for statement_input, statement_type in self._STATEMENT_SPECS:
            try:
                payload = self._call_financial_report_route(
                    ...
                    raw_symbol=raw_symbol,
                    statement_input=statement_input,
                    indicator_input=self._STATEMENT_ROUTE_INPUT,
                )
            except Exception as exc:
                if self._is_hk_financial_route_unavailable(exc):
                    raise RuntimeError(
                        "AKShare HK financial statements route unavailable: "
                        f"{self._STATEMENT_ROUTE_NAME}(symbol={statement_input}, "
                        f"indicator={self._STATEMENT_ROUTE_INPUT}) -> "
                        f"{type(exc).__name__}: {exc}"
                    ) from exc
                raise
            ...
            for row_idx, row in enumerate(rows):
                report_period_end = self._normalize_date(
                    self._pick(row, row_idx, "REPORT_DATE", "报告期"),
                    field_name="report_period_end",
                )
                period_type = self._resolve_period_type(
                    date_type_code=self._pick(row, row_idx, "DATE_TYPE_CODE", "date_type_code"),
                    report_period_end=report_period_end,
                )
                identity = (report_period_end, statement_type, period_type)
                existing = records_by_identity.get(identity)
                ...
                if source_ts is not None:
                    normalized_source_ts = self._normalize_datetime(
                        source_ts,
                ...
                if currency is not None:
                    ...
                    if "currency" in existing and existing["currency"] != normalized_currency:
                        raise ValueError(
                            "Conflicting duplicate HK financial statement currency detected: "
                            f"symbol={symbol!r}, period={report_period_end!r}, "
                            f"statement_type={statement_type!r}, "
                            f"existing={existing['currency']!r}, "
                            f"candidate={normalized_currency!r}."
    ...
    def _fetch_financial_indicators(
        ...
        *,
        ...
        try:
            ...
            if self._is_hk_financial_route_unavailable(exc):
                raise RuntimeError(
                    "AKShare HK financial indicators route unavailable: "
                    f"{self._INDICATOR_ROUTE_NAME}(indicator={self._INDICATOR_ROUTE_INPUT}) -> "
                    f"{type(exc).__name__}: {exc}"
                ) from exc
            raise
        ...
        for row_idx, row in enumerate(rows):
            ...
            source_ts = self._pick_optional(row, "REPORT_DATE", "STD_REPORT_DATE")
            normalized_source_ts = None
            if source_ts is not None:
                normalized_source_ts = self._normalize_datetime(source_ts, field_name="source_ts")
            ...
            for metric_code in sorted(row.keys()):
                if metric_code in self._FINANCIAL_META_FIELDS:
                    continue
                ...
                if float(existing["metric_value"]) != metric_value:
                    raise ValueError(
                        "Conflicting duplicate HK financial indicator row detected: "
                        f"symbol={symbol!r}, period={report_period_end!r}, "
                        f"period_type={period_type!r}, metric_code={metric_code!r}, "
                        f"existing={existing['metric_value']!r}, candidate={metric_value!r}."
                    )
    ...
    def _resolve_fetch_financial_report(self) -> Callable[..., Any]:
        ...
        try:
            ...
            raise RuntimeError(
                "akshare dependency is required for live AKShare HK financial-data fetch."
            ) from exc

        if hasattr(ak, self._STATEMENT_ROUTE_NAME):
            return getattr(ak, self._STATEMENT_ROUTE_NAME)
        raise RuntimeError(
            "AKShare HK financial statements function is unavailable: "
            f"{self._STATEMENT_ROUTE_NAME}"
        )
    ...
    def _resolve_fetch_financial_indicator(self) -> Callable[..., Any]:
        ...
        if hasattr(ak, self._INDICATOR_ROUTE_NAME):
            return getattr(ak, self._INDICATOR_ROUTE_NAME)
        raise RuntimeError(
            "AKShare HK financial indicators function is unavailable: "
            f"{self._INDICATOR_ROUTE_NAME}"
        )
    ...
    def _call_financial_report_route(
        ...
        *,
        ...
        stock_arg = self._resolve_supported_arg(
            accepted_args=accepted_args,
            supports_var_kwargs=supports_var_kwargs,
            candidates=("stock", "code", "symbol"),
            route_name=self._STATEMENT_ROUTE_NAME,
            field_label="stock/code",
        )
        kwargs[stock_arg] = raw_symbol

        statement_arg = self._resolve_supported_arg(
            accepted_args=accepted_args,
            supports_var_kwargs=supports_var_kwargs,
            candidates=("symbol", "statement", "report_type", "table"),
            route_name=self._STATEMENT_ROUTE_NAME,
            field_label="statement symbol",
        )
        kwargs[statement_arg] = statement_input
        ...
        *,
        ...
        for arg_name in candidates:
            if self._supports_arg(
                ...
                return arg_name
        raise RuntimeError(
            "AKShare HK financial-data route does not accept required argument: "
            f"route={route_name}, field={field_label}"
        )
    ...
    def _payload_to_rows(
        ...
        *,
        ...
        if not isinstance(candidate, list):
            raise ValueError(
                "AKShare HK financial-data payload must be DataFrame-like or "
                f"list[Mapping], got {type(payload).__name__}, route={route_name}."
            )
        ...
        for idx, row in enumerate(candidate):
            if not isinstance(row, Mapping):
                raise ValueError(
                    "AKShare HK financial-data payload row must be mapping. "
                    f"route={route_name}, idx={idx}, got={type(row).__name__}."
                )
            rows.append(row)
        return rows
    ...
    def _normalize_hk_symbol(self, value: str) -> tuple[str, str]:
        if "." in value:
            ...
            if market != "HK":
                raise ValueError(
                    "Unsupported symbol market suffix for HK financial-data adapter: "
                    f"{market!r}. Expected '.HK'."
                )
        else:
            code = value
        ...
        if not code.isdigit():
            raise ValueError(
                f"Unsupported HK symbol format: {value!r}. "
                "Expected canonical like '00700.HK' or raw numeric stock code."
            )
        if len(code) < 1 or len(code) > 5:
            raise ValueError(
                f"Unsupported HK symbol format: {value!r}. "
                "Expected 1-5 digit HK stock code."
            )
        ...
        *keys: str,
        ...
        for key in keys:
            if key in row:
                return row[key]
        raise ValueError(
            "Missing required source field in HK financial-data row "
            f"{row_idx}: one of {keys!r}."
        )
    ...
    def _resolve_period_type(
        ...
        *,
        ...
        date_type = self._normalize_date_type_code(date_type_code)
        if date_type == "001":
            return "annual"
    ...
    def _normalize_date_type_code(self, value: Any) -> str:
        if isinstance(value, bool):
            raise ValueError("Invalid date_type_code value type: bool")
        if isinstance(value, int):
            return f"{value:03d}"
        if isinstance(value, float):
            if not value.is_integer():
                raise ValueError(f"Invalid date_type_code value: {value!r}")
            return f"{int(value):03d}"
        if isinstance(value, str):
            stripped = value.strip()
            if stripped == "":
                raise ValueError("Invalid date_type_code value: empty string")
            if not stripped.isdigit():
                raise ValueError(f"Invalid date_type_code value: {value!r}")
            return stripped.zfill(3)
        raise ValueError(f"Invalid date_type_code value type: {type(value).__name__}")

    def _resolve_statement_metric_field(
        self,
        *,
        statement_type: str,
        item_name: Any | None,
        ...
        if not isinstance(item_name, str):
            raise ValueError(
                "Invalid STD_ITEM_NAME value type for HK financial statement row: "
                f"{type(item_name).__name__}"
            )
    ...
    def _resolve_indicator_unit(
        ...
        *,
        ...
        if metric_code in self._PERCENT_LIKE_FIELDS:
            return "percent"
        if metric_code in self._CURRENCY_PER_SHARE_FIELDS:
            if currency is None:
                return "currency_per_share"
    ...
    def _normalize_currency(self, value: Any) -> str:
        if not isinstance(value, str):
            raise ValueError(
                "Invalid currency value type for HK financial-data adapter: "
                f"{type(value).__name__}"
            )
        currency = value.strip().upper()
        if currency == "":
    ...
    def _is_hk_financial_route_unavailable(self, exc: BaseException) -> bool:
        network_exception_names = {
            "ProxyError",
            "ConnectionError",
            "ConnectTimeout",
...
class AkshareIndexDailyBarAdapter:
    ...
    _DEFAULT_ROUTE_NAME = "custom_fetch_index_daily"
    ...
    def fetch(
        ...
        *,
        ...
        for canonical_code, akshare_symbol, code in requested_symbols:
            ...
            if len(records) == 0:
                raise ValueError(
                    "AKShare index daily-bar request yielded no usable rows for "
                    f"requested symbol: {canonical_code!r} within bounded window "
                    f"{bounded_start.isoformat()}..{bounded_end.isoformat()}."
                )
    ...
    def _fetch_index_rows(
        ...
        *,
        ...
        start_arg = self._find_supported_arg(
            accepted_args=accepted_args,
            supports_var_kwargs=supports_var_kwargs,
            candidates=("start_date", "begin_date", "date_start", "start"),
        )
        if start_arg is not None:
            kwargs[start_arg] = self._to_akshare_date(start_date)

        end_arg = self._find_supported_arg(
            accepted_args=accepted_args,
            supports_var_kwargs=supports_var_kwargs,
            candidates=("end_date", "finish_date", "date_end", "end"),
        )
        if end_arg is not None:
            kwargs[end_arg] = self._to_akshare_date(end_date)

        if route_name == "index_zh_a_hist":
            period_arg = self._resolve_required_arg(
                accepted_args=accepted_args,
                supports_var_kwargs=supports_var_kwargs,
                candidates=("period", "freq", "frequency"),
                route_name=route_name,
    ...
    def _resolve_bounded_date_window(
        ...
        *,
        ...
        if start_date is None and end_date is None:
            raise ValueError(
                "AkshareIndexDailyBarAdapter requires bounded date window via "
                "start_date and end_date."
            )
        if start_date is None or end_date is None:
            raise ValueError(
                "AkshareIndexDailyBarAdapter requires both start_date and end_date "
                "for a bounded date-window request."
            )
        if start_date > end_date:
            raise ValueError(
                "Invalid date range for AkshareIndexDailyBarAdapter: "
                f"start_date={start_date.isoformat()} > end_date={end_date.isoformat()}"
            )
        return start_date, end_date
    ...
    def _normalize_index_symbol(
        ...
        *,
        ...
        if normalized == "":
            raise ValueError(
                "Invalid index symbol value for index daily-bars adapter: empty string."
            )
        ...
        if normalized.startswith(("SH", "SZ")) and len(normalized) == 8:
            ...
            if not code.isdigit():
                raise ValueError(
                    f"Unsupported source-native index symbol format: {symbol!r}."
                )
            mapped = _CN_INDEX_AKSHARE_SYMBOL_MAP.get(code)
            expected = f"{prefix.lower()}{code}"
            if mapped is None:
        ...
        if "." in normalized:
            ...
            if market == "CN_INDEX":
                ...
                raise ValueError(
                    f"Hong Kong stock symbol is unsupported for index daily-bars adapter: {symbol!r}."
                )
            elif market in {"ETF_CN", "FUND_CN"}:
                raise ValueError(
                    f"ETF/fund symbol is unsupported for index daily-bars adapter: {symbol!r}."
                )
            else:
                raise ValueError(
    ...
    def _raise_unsupported_index_symbol(self, *, symbol: Any, code: str) -> None:
        ...
        if code.startswith(("4", "6", "8", "9")):
            raise ValueError(
                f"A-share stock-like symbol is unsupported for index daily-bars adapter: {symbol!r}."
            )
        if code.startswith(("00", "30")) and code not in _CN_INDEX_AKSHARE_SYMBOL_MAP:
            raise ValueError(
                "Unsupported or unmapped benchmark index code for current adapter slice: "
                f"{code!r}."
            )
        if code.startswith("399"):
            raise ValueError(
        ...
        raise ValueError(
            "Unsupported index symbol for current adapter slice: "
            f"{symbol!r}."
        )

    def _resolve_route_symbol(
        self,
        *,
        route_name: str,
        akshare_symbol: str,
    ...
    def _normalize_index_rows(
        ...
        *,
        ...
        for idx, row in enumerate(rows):
            ...
            if existing == record:
                continue
            normalized_by_trade_date[trade_date] = self._merge_duplicate_record(
                existing=existing,
                candidate=record,
            )
    ...
    def _merge_duplicate_record(
        ...
        *,
        ...
        for field_name in comparable_fields:
            ...
            if left is None or right is None:
                raise ValueError(
                    "Conflicting duplicate index daily-bar row detected: "
                    f"field={field_name!r}, existing={left!r}, candidate={right!r}."
                )
            if isinstance(left, (int, float)) and isinstance(right, (int, float)):
                if float(left) != float(right):
        ...
        if merged.get("source_ts") is None and candidate_source_ts is not None:
            ...
            isinstance(merged.get("source_ts"), str)
            and isinstance(candidate_source_ts, str)
            and candidate_source_ts > merged["source_ts"]
        ):
            merged["source_ts"] = candidate_source_ts
        return merged
    ...
    def _validate_records(
        ...
        *,
        ...
        for idx, record in enumerate(records):
            ...
            if issues:
                raise ValueError(
                    "Invalid normalized index daily-bar record: "
                    f"idx={idx}, issues={issues!r}"
                )
    ...
    def _is_index_daily_route_unavailable(self, exc: BaseException) -> bool:
        network_exception_names = {
            "ProxyError",
            "ConnectionError",
            "ConnectTimeout",
...
class AkshareETFFundNavSnapshotAdapter:
    """AKShare adapter for caller-provided ETF/fund NAV batches."""
    ...
    def fetch(
        ...
        *,
        ...
        requested_symbols = self._require_symbols(symbols)
        self._validate_requested_date_window(
            start_date=start_date,
            end_date=end_date,
        ...
        for canonical_fund_code, akshare_fund_code in requested_symbols:
            ...
            if len(requested_symbols) > 1 and not filtered_records:
                raise ValueError(
                    "AKShare ETF/fund NAV request yielded no usable rows for requested "
                    f"symbol: {canonical_fund_code!r}."
                )
            normalized_records.extend(filtered_records)
    ...
    def _validate_requested_date_window(
        ...
        *,
        ...
        if start_date is not None and end_date is not None and end_date < start_date:
            raise ValueError(
                "Invalid date range for AkshareETFFundNavSnapshotAdapter: "
                f"start_date={start_date.isoformat()} > end_date={end_date.isoformat()}"
            )
        if symbol_count <= 1:
            return
        if start_date is None and end_date is None:
            raise ValueError(
                "AkshareETFFundNavSnapshotAdapter requires bounded date window via "
                "start_date and end_date for multi-symbol requests."
            )
        if start_date is None or end_date is None:
            raise ValueError(
                "AkshareETFFundNavSnapshotAdapter requires both start_date and "
                "end_date for multi-symbol bounded date-window requests."
            )
    ...
    def _normalize_fund_nav_rows(
        ...
        *,
        ...
        normalized: list[dict[str, Any]] = []
        first_missing_required_error: ValueError | None = None
        ingested_at = self._now_fn().isoformat()
        schema_version = self._registry.get(dataset).schema_version
        ...
        for idx, row in enumerate(rows):
            try:
                record: dict[str, Any] = {
                    ...
                    "nav": self._to_float(
                        self._pick(row, idx, "单位净值", "nav", "unit_nav")
                    ),
                    "source": AKSHARE_SOURCE_ID,
                    "ingested_at": ingested_at,
                    "schema_version": schema_version,
                ...
                if "Missing required source field" not in str(exc):
                    raise
                if first_missing_required_error is None:
                    first_missing_required_error = exc
                continue
    ...
    def _merge_duplicate_record(
        ...
        *,
        ...
        for field in (
            ...
            if existing.get(field) != candidate.get(field):
                raise ValueError(
                    "Conflicting duplicate ETF/fund NAV row detected: "
                    f"field={field!r}, existing={existing.get(field)!r}, "
        ...
        for field in ("accumulated_nav", "shares_outstanding", "fund_scale"):
            existing_value = merged.get(field)
            candidate_value = candidate.get(field)
            if existing_value is None and candidate_value is not None:
                merged[field] = candidate_value
...
class AkshareETFFundPremiumDiscountAdapter:
    """Bounded AKShare adapter for ETF/fund premium-discount source facts."""

    source_name = AKSHARE_SOURCE_ID
    source_display_name = AKSHARE_SOURCE_NAME

    _PRIMARY_ROUTE_NAME = "fund_etf_fund_daily_em"
    _FALLBACK_ROUTE_NAME = "fund_etf_spot_em"
    _DAILY_SOURCE_CATEGORY = "exchange_traded_fund_daily_summary"
    _SPOT_SOURCE_CATEGORY = "exchange_etf_realtime_quote"

    def __init__(
        self,
        *,
        fetch_fund_daily: Callable[..., Any] | None = None,
        fetch_etf_spot: Callable[..., Any] | None = None,
        now_fn: Callable[[], datetime] | None = None,
    ) -> None:
        self._fetch_fund_daily = fetch_fund_daily
        self._fetch_etf_spot = fetch_etf_spot
        self._now_fn = now_fn or (lambda: datetime.now(timezone.utc))
        self._registry = DatasetRegistry()
    ...
    def fetch(
        ...
        *,
        ...
        if dataset != DatasetName.FUND_PREMIUM_DISCOUNT:
            raise ValueError(
                "Unsupported dataset for AkshareETFFundPremiumDiscountAdapter: "
                f"{dataset.value}"
            )
        ...
        requested_symbols = self._require_symbols(symbols)
        route_name, rows = self._fetch_rows_for_window()
        records = self._normalize_premium_discount_rows(
            rows=rows,
            requested_funds={
                raw_fund_code: canonical_fund_code
                for canonical_fund_code, raw_fund_code in requested_symbols
            },
            dataset=dataset,
            route_name=route_name,
            start_date=window_start,
        ...
        records = self._dedupe_and_sort_records(records)
        self._assert_no_partial_batch_success(
            requested_symbols=requested_symbols,
            records=records,
        )
    ...
    def _fetch_rows_for_window(self) -> tuple[str, list[Mapping[str, Any]]]:
        primary_fetch = self._resolve_fetch_fund_daily()
        try:
            primary_payload = primary_fetch()
        except Exception as primary_exc:
            if not self._is_fund_premium_discount_route_unavailable(primary_exc):
                raise

            fallback_fetch = self._resolve_fetch_etf_spot()
            try:
                fallback_payload = fallback_fetch()
            except Exception as fallback_exc:
                if self._is_fund_premium_discount_route_unavailable(fallback_exc):
                    raise RuntimeError(
                        "AKShare ETF/fund premium-discount routes unavailable: "
                        f"primary={self._PRIMARY_ROUTE_NAME} -> {type(primary_exc).__name__}: {primary_exc}; "
                        f"fallback={self._FALLBACK_ROUTE_NAME} -> {type(fallback_exc).__name__}: {fallback_exc}"
                    ) from fallback_exc
                raise
    ...
    def _resolve_fetch_fund_daily(self) -> Callable[..., Any]:
        if self._fetch_fund_daily is not None:
            return self._fetch_fund_daily

        try:
            import akshare as ak  # type: ignore[import-not-found]
        except Exception as exc:  # pragma: no cover - exercised by live/dependency env
            raise RuntimeError(
                "akshare dependency is required for live AKShare ETF/fund premium-discount fetch."
            ) from exc

        if hasattr(ak, self._PRIMARY_ROUTE_NAME):
            return getattr(ak, self._PRIMARY_ROUTE_NAME)
        raise RuntimeError(
            "AKShare ETF/fund premium-discount primary function is unavailable in this "
            f"akshare version: {self._PRIMARY_ROUTE_NAME}"
        )

    def _resolve_fetch_etf_spot(self) -> Callable[..., Any]:
        if self._fetch_etf_spot is not None:
            return self._fetch_etf_spot
        ...
        if hasattr(ak, self._FALLBACK_ROUTE_NAME):
            return getattr(ak, self._FALLBACK_ROUTE_NAME)
        raise RuntimeError(
            "AKShare ETF/fund premium-discount fallback function is unavailable in this "
            f"akshare version: {self._FALLBACK_ROUTE_NAME}"
        )
    ...
    def _resolve_bounded_trade_window(
        ...
        *,
        ...
        if start_date is None and end_date is None:
            raise ValueError(
                "AkshareETFFundPremiumDiscountAdapter requires bounded trade_date via "
                "start_date and end_date."
            )
        if start_date is None or end_date is None:
            raise ValueError(
                "AkshareETFFundPremiumDiscountAdapter requires both start_date and "
                "end_date for bounded trade-date window requests."
            )
        if start_date > end_date:
            raise ValueError(
                "Invalid date range for AkshareETFFundPremiumDiscountAdapter: "
                f"start_date={start_date.isoformat()} > end_date={end_date.isoformat()}"
            )
        return start_date, end_date
    ...
    def _require_symbols(
        ...
        if symbols is None or len(symbols) == 0:
            raise ValueError(
                "AkshareETFFundPremiumDiscountAdapter requires at least one ETF/fund "
                "code, got none."
            )
        normalized_symbols: list[tuple[str, str]] = []
        seen: set[str] = set()
    ...
    def _normalize_symbol(
        ...
        *,
        ...
        if "." in normalized:
            ...
            if market != "ETF_CN":
                raise ValueError(
                    "Unsupported ETF/fund premium-discount market suffix: "
                    f"{market!r}. Expected '.ETF_CN'."
                )
        else:
            code = normalized
        ...
        if not code.isdigit() or len(code) != 6:
            raise ValueError(
                f"Unsupported ETF/fund premium-discount code format: {normalized!r}. "
                "Expected 6-digit code like '510300' or '510300.ETF_CN'."
            )
        if code in _CN_INDEX_AKSHARE_SYMBOL_MAP or code.startswith("399"):
            raise ValueError(
                "Index code is unsupported for ETF/fund premium-discount adapter: "
                f"{code!r}."
            )
        if code.startswith(("6", "0", "3", "4", "8", "9")):
            raise ValueError(
                "A-share stock code is unsupported for ETF/fund premium-discount "
                f"adapter: {code!r}."
            )
        if not code.startswith(("5", "1")):
            raise ValueError(
    ...
    def _payload_to_rows(
        ...
        *,
        ...
        if not isinstance(candidate, list):
            raise ValueError(
                "AKShare ETF/fund premium-discount payload must be DataFrame-like or "
                f"list[Mapping], got {type(payload).__name__}, route={route_name}."
            )
        ...
        for idx, row in enumerate(candidate):
            if not isinstance(row, Mapping):
                raise ValueError(
                    "AKShare ETF/fund premium-discount payload row must be mapping. "
                    f"route={route_name}, idx={idx}, got={type(row).__name__}."
                )
            rows.append(row)
        return rows

    def _normalize_premium_discount_rows(
        self,
        *,
        rows: Sequence[Mapping[str, Any]],
        requested_funds: Mapping[str, str],
        ...
        if route_name == self._PRIMARY_ROUTE_NAME:
            return self._normalize_daily_rows(
                rows=rows,
        ...
        if route_name == self._FALLBACK_ROUTE_NAME:
            return self._normalize_spot_rows(
                rows=rows,
                requested_funds=requested_funds,
                dataset=dataset,
                route_name=route_name,
        ...
        raise RuntimeError(
            f"Unsupported ETF/fund premium-discount route name: {route_name!r}"
        )
    ...
    def _normalize_daily_rows(
        ...
        *,
        ...
        trade_date, nav_column = self._resolve_daily_nav_column(rows)
        trade_dt = date.fromisoformat(trade_date)
        if trade_dt < start_date or trade_dt > end_date:
            return []
        ...
        for row_idx, row in enumerate(rows):
            ...
            try:
                market_price = self._to_float(
                    self._pick(row, row_idx, "市价", "market_price"),
                    field_name="market_price",
                )
                nav = self._to_float(
                    self._pick(row, row_idx, nav_column),
                    field_name="nav",
                )
                premium_discount_rate = self._to_percentage_float(
                    self._pick(row, row_idx, "折价率", "premium_discount_rate"),
            ...
            record: dict[str, Any] = {
                ...
                "premium_discount_amount": market_price - nav,
                "source_route": route_name,
                "source_category": self._DAILY_SOURCE_CATEGORY,
                "source": AKSHARE_SOURCE_ID,
                "ingested_at": ingested_at,
                "schema_version": schema_version,
            }
    ...
    def _normalize_spot_rows(
        ...
        *,
        ...
        for row_idx, row in enumerate(rows):
            row_code = self._normalize_source_fund_code(
                self._pick(row, row_idx, "代码", "fund_code", "code"),
                field_name="fund_code",
            )
            canonical_fund_code = requested_funds.get(row_code)
            if canonical_fund_code is None:
            ...
            try:
                trade_date = self._normalize_trade_date(
                    self._pick(row, row_idx, "数据日期", "trade_date", "date"),
                    field_name="trade_date",
                )
                trade_dt = date.fromisoformat(trade_date)
                if trade_dt < start_date or trade_dt > end_date:
                ...
                market_price = self._to_float(
                    self._pick(row, row_idx, "最新价", "market_price"),
                    field_name="market_price",
                )
                premium_discount_rate = self._to_percentage_float(
                    self._pick(row, row_idx, "基金折价率", "premium_discount_rate"),
                    field_name="premium_discount_rate",
                )
            except ValueError as exc:
                if "Missing required source field" not in str(exc):
            ...
            record = {
                ...
                "premium_discount_rate": premium_discount_rate,
                "source_route": route_name,
                "source_category": self._SPOT_SOURCE_CATEGORY,
                "source": AKSHARE_SOURCE_ID,
                "ingested_at": ingested_at,
                "schema_version": schema_version,
            }

            iopv = self._pick_optional(row, "IOPV实时估值", "iopv")
            if iopv is not None:
                iopv_value = self._to_float(iopv, field_name="iopv")
                record["iopv"] = iopv_value
                record["premium_discount_amount"] = market_price - iopv_value

            source_ts = self._pick_optional(row, "更新时间", "source_ts", "update_time")
            if source_ts is not None:
                record["source_ts"] = self._normalize_source_ts(source_ts)
    ...
    def _resolve_daily_nav_column(
        self,
        rows: Sequence[Mapping[str, Any]],
    ) -> tuple[str, str]:
        nav_columns: list[tuple[date, str]] = []
        for row in rows:
            for key in row:
                if not isinstance(key, str):
                    continue
                match = re.match(r"^(\d{4}-\d{2}-\d{2})-单位净值$", key.strip())
                if match is None:
                    continue
                nav_columns.append((date.fromisoformat(match.group(1)), key))

        if not nav_columns:
            raise ValueError(
                "AKShare ETF/fund premium-discount daily route missing dated NAV column."
            )
        latest_trade_date, nav_column = max(nav_columns, key=lambda item: item[0])
        return latest_trade_date.isoformat(), nav_column
    ...
    def _merge_duplicate_record(
        ...
        *,
        ...
        for field in (
            ...
            if existing.get(field) != candidate.get(field):
                raise ValueError(
                    "Conflicting duplicate ETF/fund premium-discount row detected: "
    ...
    def _assert_no_partial_batch_success(
        self,
        *,
        requested_symbols: Sequence[tuple[str, str]],
        records: Sequence[Mapping[str, Any]],
    ) -> None:
        if not records:
            return
        returned_symbols = {str(record["fund_code"]) for record in records}
        for canonical_symbol, _ in requested_symbols:
            if canonical_symbol not in returned_symbols:
                raise ValueError(
                    "AKShare ETF/fund premium-discount request yielded no usable rows "
                    f"for requested symbol: {canonical_symbol!r}."
                )
        ...
        *keys: str,
        ...
        for key in keys:
            ...
            if not self._is_missing_value(value):
                return value
        raise ValueError(
            "Missing required source field in ETF/fund premium-discount row "
            f"{row_idx}: one of {keys!r}."
        )
    ...
    def _to_percentage_float(self, value: Any, *, field_name: str) -> float:
        return self._to_float(value, field_name=field_name)

    def _is_fund_premium_discount_route_unavailable(self, exc: BaseException) -> bool:
        network_exception_names = {
            "ProxyError",
            "ConnectionError",
            "ConnectTimeout",
        ...
        while current is not None and id(current) not in seen:
            ...
            if name in network_exception_names:
                return True
            if module.startswith(("requests", "urllib3", "http.client")) and any(
                token in message for token in network_message_tokens
            ):
                return True
            if isinstance(current, (socket.timeout, TimeoutError, ConnectionError, ssl.SSLError)):
