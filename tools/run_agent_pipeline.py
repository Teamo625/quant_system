...
def build_controller_packet(task: ActiveTask) -> Path:
    ...
    content = [
        ...
        "",
        "Generated by `tools/run_agent_pipeline.py` after Integration.",
        "",
        ...
        "- Do not redo implementation-level code review; Review and Integration records are the source of that judgment.",
        "",
        "## Task",
        "",
        ...
        f"- integration: `{rel(task.integration)}`",
        "",
        ...
        "## Review Digest",
        "",
        compact_file_digest(task.review, ["Decision", "Findings (ordered by severity)", "Residual Risk", "Follow-up Requirements"], max_chars=5000),
        "",
        "## Integration Digest",
        "",
        compact_file_digest(task.integration, ["Integration Result", "Conflict / Scope Check", "Residual Risk", "State Update Recommendations"], max_chars=6000),
        "",
        "## Phase Gate Rules",
        "",
...
职责边界：
- 不要重新做实现级代码审查；代码正确性由 Review Agent 和 Integration Agent 的本地记录负责。
